<template>
    <div id="app" class="components-slider">
      <div class="components-slider-rail" ref="rail">
        <div
          ...
          :class="{ 'components-slider-process--transition': !dragging.min && !dragging.max }"
          :style="{ width: max - min + '%', left: min + '%', right: 100 - max + '%' }"
        ></div>
        <div
  ...
  const clickChange = (evt: MouseEvent) => {
    ...
    if (betweenMinAndMax) {
      filterChangeType = Math.abs(max.value - percent) > Math.abs(min.value - percent);
    }
